...
router.post("/worker", async function (req, res, next) {
  ...
  await connectionBD.query(sql, function (error, results) {
    if (error) {
      ...
      if (results[0] == null) {
        band = 1;
        let sql = `INSERT INTO trabajadores (id,email,tipo_documento,nombres,apellidos,fecha_nacimiento,password,tipo_servicio,tarifa_hora,telefono, detalle_servicio)
        VALUES ('${id}','${email}','${tipoDoc}','${nombres}','${apellidos}','${fechaNacimiento}',
        '${password}','${tipoServicio}',${tarifaHora}, ${phone}, '${detalleServicio}')`;
        connectionBD.query(sql, function (error, results) {
          if (error) {
            console.log(error);
          } else {
